...
- **An iOS home-screen app is a separate storage partition.** Nothing saved while running in Safari (localStorage, cookies, Cache Storage) exists in the installed app; only passkeys (iCloud Keychain) and the manifest's `start_url` cross over. Two things lean on that (2026-09-05):
  - `functions/manifest.json.js` serves the manifest with `start_url=/?server=<url>&u=<username>` taken from the `<link rel="manifest">` query (`?s=&u=`, rewritten by `ABS.setInstallHint`, called from `saveCredentials`) or the `pholia_install` cookie — both, because it isn't documented which Safari reads at Add-to-Home-Screen time. `App._consumeInstallHint()` (first thing in `tryAutoLogin`) adopts it as the login-form prefill when the partition has no `pholia_server`, then strips it from the URL. `sw.js` strips `server`/`u` from the shell cache key so an offline launch still hits the precached `./`. The manifest has `"id": "/"` so the changing manifest URL is still one app. **The token is never put in `start_url`** — it would live in a launch URL forever.
