...
**A multi-part mp3 book plays as ONE resource** (`Player._useBookStream` → the shim's `/api/items/:id/stream`). iOS will not start a new media load while the app is backgrounded — at a track boundary the element takes the metadata, reports itself playing, lets the lock screen tick and makes no sound until the app is opened. It happens whether the bytes come from the worker or straight from the network, so the block is on *starting a load*, not on who serves it; the `native` pin below was not enough on its own. One resource has no boundaries, so nothing is ever loaded again after playback starts. Only mp3 (concatenation relies on the decoder walking frames straight through) and **not for a downloaded book**, whose cache is keyed per part. The scrubber and times are unaffected: they come from `chapters` and global time, never from the track layout.

**A load that STARTS while the app is backgrounded is pinned `native` (2026-09-06).** A worker-synthesized body is a JS ReadableStream and WebKit stops pulling from it while the page is hidden, so at a track boundary in the background the element takes its metadata, fires `playing`, lets the lock screen tick — and makes no sound, then restarts the track from zero when the app is opened. The crash log is unambiguous: the worker enqueued all 33.8 MB in 64 ms at 23:57:45, the element sat at `readyState 1` for 25 s, and `canplay` arrived at 23:58:11, the moment the app came to the foreground. `pinMediaMode` sends `hidden`/`online` and `modeFor` short-circuits on them. **Offline is the exception** — with no network the worker is the only source, so a hidden load stays `sw`. This costs a cached book some bandwidth when a track turns over in your pocket, which beats silence.
