...
**The memo does not help a cold launch, and the numbers say so.** It lives in page memory, so every launch still runs the scan once, and the home produce awaits it *before* awaiting `/personalized`. Phone marks on 464e663 (2026-09-20 08:52 UTC): home-cached=1076, libraries=1363, scan=4523, personalized=4523, home=4526 — `scan` and `personalized` at the same millisecond means the shelves data had been sitting there waiting for the scan. Cached shelves are on screen in ~60–160 ms so the launch *feels* instant, but the fresh paint (and the dot on a book downloaded since the last launch) still lands at ~4.5–5 s. The fix that has not been built yet: paint the shelves from `/personalized` as soon as it lands and add the Downloaded row / dots when the scan finishes, or persist the scan's ids with a version stamp that chunk writes also persist. Making the scan itself faster is not an option — Cache Storage `keys()` over every chunk is what costs, and that is iOS.
