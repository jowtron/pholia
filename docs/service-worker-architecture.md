...
**All-or-nothing per file.** iOS's media loader cancels a resource whose CORS status changes between responses, and a worker-synthesized response (`Access-Control-Allow-Origin: *`) and a native no-cors response have different statuses. So the page announces every media load (`MEDIA_LOAD` → `App.pinMediaMode`) and the worker pins a mode per file (`modeFor`): `sw` (cached parts from Cache Storage, everything else bridged with a CORS fetch carrying only `Range`) or `native` (the worker never answers). Pins persist with the flags. Serving a partially cached file while letting the tail go native is exactly the 2026-09-03 failure (tail re-requested every 170 ms, then the loader gave up).
